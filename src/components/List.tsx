...
export function List({ items, handleItems, deleteItem }: ListProps) {
	return (
		<div data-testid={'task_list_container'} className={styles.taskList}>
			<div className={styles.taskListHeader}>
				...
				<div data-testid={'list_completion_track'} className={styles.taskListConcludeList}>
					<p>
						Concluídas{' '}
						<span>
							{items.filter((el) => el.checked).length} de {items.length}
						</span>
					</p>
				</div>
			</div>
			{items.length ? (
				items.map((el) => (
					<div key={el.id} data-testid={`list_item_container_${el.id}`} className={styles.taskListItemContainer}>
						<div className={styles.taskListItemCheckBoxContainer}>
							<input
								type="checkbox"
								value={el.id}
								name={el.id}
							...
							/>
							<label htmlFor={el.id} className={styles.taskListItemLabel}>
								{el.content}
							</label>
						</div>
						<time
